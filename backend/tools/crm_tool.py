"""CRM tool — thread-safe access to sample CRM data."""
...
_lock = Lock()
_crm_cache: Optional[List[Dict[str, Any]]] = None
_usage_cache: Optional[List[Dict[str, Any]]] = None
_updates: Dict[str, Dict[str, Any]] = {}


def _load_crm_data() -> List[Dict[str, Any]]:
    global _crm_cache
    with _lock:
        if _crm_cache is None:
            try:
                ...
                _crm_cache = []
        return list(_crm_cache)  # Return a shallow copy of the list
...
def get_all_accounts() -> List[Dict[str, Any]]:
    ...
    with _lock:
        updates_snapshot = copy.deepcopy(_updates)
    for acc in accounts:
        merged = copy.deepcopy(acc)
        if acc["account_id"] in updates_snapshot:
            merged.update(updates_snapshot[acc["account_id"]])
...
def get_accounts_by_stage(stage: str) -> List[Dict[str, Any]]:
    return [a for a in get_all_accounts() if a.get("stage", "").lower() == stage.lower()]


def get_at_risk_deals(inactivity_days: int = 10) -> List[Dict[str, Any]]:
...
def get_usage_data(account_id: str) -> Optional[Dict[str, Any]]:
...
def update_deal_stage(account_id: str, new_stage: str, notes: str = "") -> Dict[str, Any]:
    with _lock:
...
def log_activity(account_id: str, activity_type: str, description: str) -> Dict[str, Any]:
    with _lock:
        if account_id not in _updates:
            _updates[account_id] = {}
        _updates[account_id].update({
...
def add_new_lead(lead_data: Dict[str, Any]) -> Dict[str, Any]:
    global _crm_cache
    with _lock:
        if _crm_cache is None:
            _crm_cache = []
