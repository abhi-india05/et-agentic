...
def _fallback_explanation(task_type: str, summaries: Dict[str, Any]) -> ExplainabilityOutput:
    successful = [name for name, item in summaries.items() if item.get("status") == "success"]
    failed = [name for name, item in summaries.items() if item.get("status") == "failure"]
    return ExplainabilityOutput(
        executive_summary=(
            f"RevOps AI completed a {task_type} workflow. "
            f"{len(successful)} agents succeeded" + (f" and {len(failed)} failed." if failed else ".")
        ),
        decision_chain=[
            {
                "step": index + 1,
                "agent": name,
                "decision": item.get("reasoning", "")[:120],
                "why": "Part of the automated workflow",
                "confidence": item.get("confidence", 0.5),
                "impact": "Contributed to the overall workflow output",
            }
            for index, (name, item) in enumerate(summaries.items())
        ],
        key_insights=["Automated pipeline executed", f"Task type: {task_type}"],
        data_sources_used=["CRM data", "LLM completions", "Vector memory"],
        overall_confidence=sum(item.get("confidence", 0.0) for item in summaries.values()) / max(len(summaries), 1),
        limitations=["LLM outputs may require human validation"],
        human_review_recommended=bool(failed),
        human_review_reasons=[f"Agents failed: {', '.join(failed)}"] if failed else [],
        impact_metrics={"actions_taken": len(successful)},
    )
...
def run_explainability_agent(session_id: str, agent_outputs: Dict[str, Any], task_type: str) -> Dict[str, Any]:
    ...
    try:
        explanation = _call_llm(
            f"""You are an AI explainability specialist. Generate a clear explanation of what the RevOps AI system did and why.
